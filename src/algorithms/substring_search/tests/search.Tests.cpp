...
TEST(KnutMorrisPrattAlgorithmTests, SimpleTest) {
    std::string txt = "ABCDEABCDABCE";
    std::string pat = "ABCD";
    std::vector<size_t> arr;

    searchKnuttMorisPratt(txt, pat, arr);
    
    ASSERT_EQ(2, arr.size());

    ASSERT_EQ(0, searchKnuttMorisPratt(txt, pat));

    txt = "BCDEABCDABCE";

    ASSERT_EQ(4, searchKnuttMorisPratt(txt, pat));

    txt = "BCDEBBCDABCE";

    ASSERT_EQ(std::string::npos, searchKnuttMorisPratt(txt, pat));
}


TEST(FiniteAutomatonAlgorithmTests, SimpleTest) {
    std::string txt = "ABCDEABCDABCE";
    std::string pat = "ABCD";
    std::vector<size_t> arr;

    finiteAutomatonSearch(txt, pat, arr);
    
    ASSERT_EQ(2, arr.size());

    ASSERT_EQ(0, finiteAutomatonSearch(txt, pat));

    txt = "BCDEABCDABCE";

    ASSERT_EQ(4, finiteAutomatonSearch(txt, pat));

    txt = "BCDEBBCDABCE";

    ASSERT_EQ(std::string::npos, finiteAutomatonSearch(txt, pat));
}

TEST(BoierMurTests, SimpleTest) {
    std::string txt = "ABCDEABCDABCE";
    std::string pat = "ABCD";
    std::vector<size_t> arr;

    searchBoierMur(txt, pat, arr);
    
    ASSERT_EQ(2, arr.size());

    ASSERT_EQ(0, searchBoierMur(txt, pat));

    txt = "BCDEABCDABCE";

    ASSERT_EQ(4, searchBoierMur(txt, pat));

    txt = "BCDEBBCDABCE";

    ASSERT_EQ(std::string::npos, searchBoierMur(txt, pat));
}
...
TEST(TimeTests, SimpleTest) {
    ...
    begin = clock();
    a = searchKnuttMorisPratt(txt, pat);
    end = clock();

    std::cout << "Knutt-Morris-Pratt: " << (double)(end - begin) / CLOCKS_PER_SEC << "s    at  " << a << std::endl;
...
TEST(TimeTests, WarAndPeaceSeveralWordsTest) {
    ...
    begin = clock();
    a = searchKnuttMorisPratt(txt, pat, arr2);
    end = clock();
...
TEST(TimeTests, ManySameLinesTest) {
    ...
    begin = clock();
    b = searchKnuttMorisPratt(txt, pat, arr2);
    end = clock();

    std::cout << "Knutt-Morris-Pratt: " << (double)(end - begin) / CLOCKS_PER_SEC << "s    at  " << b << std::endl;

    begin = clock();
    c = finiteAutomatonSearch(txt, pat, arr3);
    end = clock();

    std::cout << "Automat: " << (double)(end - begin) / CLOCKS_PER_SEC << "s    at  " << c << std::endl;

    begin = clock();
    d = searchBoierMur(txt, pat, arr4);
    end = clock();

    std::cout << "BoierMur: " << (double)(end - begin) / CLOCKS_PER_SEC << "s    at  " << d << std::endl;

    ASSERT_EQ(arr1.size(), arr2.size());
    ASSERT_EQ(arr2.size(), arr3.size());
    ASSERT_EQ(arr3.size(), arr4.size());
